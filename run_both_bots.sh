...
# Start your modified bot
echo "Starting YOUR bot in screen session 'my-bot'..."
screen -dmS my-bot bash -c "cd /root/Polymarket-Kalshi-Arbitrage-bot && RUST_LOG=info cargo run --release 2>&1 | tee logs/my-bot_${TIMESTAMP}.log"

# Start original bot
echo "Starting ORIGINAL bot in screen session 'original-bot'..."
screen -dmS original-bot bash -c "cd /root/Polymarket-Kalshi-Arbitrage-bot/original-bot && RUST_LOG=info cargo run --release 2>&1 | tee logs/original-bot_${TIMESTAMP}.log"
...
echo "  - original-bot  (terauss version)"
echo ""
echo "Log files:"
