...
impl SharedAsyncClient {
    ...
    /// Fetch orderbook for a token (public endpoint, no auth required)
    pub async fn get_orderbook(&self, token_id: &str) -> Result<OrderbookData> {
        let url = format!("{}/book?token_id={}", self.inner.host, token_id);
        ...
        if !resp.status().is_success() {
            let status = resp.status();
            let body = resp.text().await.unwrap_or_default();
            return Err(anyhow!("get_orderbook failed {}: {}", status, body));
        }
